...
def test_user_validate_name_none():
    with pytest.raises(ValidationError):
        UserModel(name=None, email="john.doe@example.com", password="ValidPass123!", role=1)


def test_user_validate_name_empty():
    with pytest.raises(ValidationError):
        UserModel(name="", email="john.doe@example.com", password="ValidPass123!", role=1)


def test_user_validate_name_too_long():
    with pytest.raises(ValidationError):
        UserModel(name="J" * 51, email="john.doe@example.com", password="ValidPass123!", role=1)
...
def test_user_validate_role_type():
    with pytest.raises(ValidationError):
        UserModel(name="John Doe", email="john.doe@example.com", password="ValidPass123!", role="hola")


def test_user_validate_role_superate_max():
    with pytest.raises(ValidationError):
        UserModel(name="John Doe", email="john.doe@example.com", password="ValidPass123!", role=4)


def test_user_validate_role_superate_min():
    with pytest.raises(ValidationError):
        UserModel(name="John Doe", email="john.doe@example.com", password="ValidPass123!", role=0)
...
def test_user_validate_phone_type():
    with pytest.raises(ValidationError):
        UserModel(name="John Doe", email="john.doe@example.com", password="ValidPass123!", role=1, phone=1237890)


# Tests validate basket/addresses
def test_user_validate_addresses_basket_valid():
    user = UserModel(name="John Doe", email="john.doe@example.com", password="ValidPass123!", role=1, basket=[{"item": 1}], addresses=[{"street": "Calle Falsa 123"}])
