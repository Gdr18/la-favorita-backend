...
def test_add_setting_duplicate_key(client, mock_db, mocker):
    ...
    assert response.json['err'] == "Error de clave duplicada en MongoDB: {'name': 'test'}"


def test_add_setting_validation_error(client, mock_db, mocker):
    error = mocker.Mock(spec=ValidationError)
    mock_db.insert_one.side_effect = error
    response = client.post('/setting', json={'name': 12345, 'values': ['value1', 'value2']})
...
def test_update_setting_not_found(client, mock_db):
    with real_app.app_context():
        mock_db.find_one.return_value = None
        response = client.put('/setting/507f1f77bcf86cd799439011')
        assert response.status_code == 404
        assert response.json['err'] == "El/la configuración con id '507f1f77bcf86cd799439011' no ha sido encontrado/a."
...
def test_update_setting_validation_error(client, mock_db, mocker):
    error = mocker.Mock(spec=ValidationError)
    mock_db.insert_one.side_effect = error
    response = client.put('/setting/507f1f77bcf86cd799439011', json={'name': 12345, 'values': ['value1', 'value2']})
    assert response.status_code == 400
    assert 'err' in response.json
