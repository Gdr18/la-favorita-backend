...
def test_user_route_unauthorized_access(client, auth_header, mock_jwt):
    request_unauthorized_access(
        client, auth_header, mock_jwt, "get", URL_USERS, {**VALID_USER_DATA, "role": 3}
    )
    request_unauthorized_access(
        client,
        auth_header,
...
def test_user_route_unauthorized_set(client, mock_db, auth_header, mock_jwt):
    request_unauthorized_set(
        client, mock_db, auth_header, mock_jwt, "post", URL_USER, VALID_USER_DATA_ROLE
    )
    request_unauthorized_set(
        client,
        mock_db,
        ...
        "put",
        URL_USER_ID,
        VALID_USER_DATA_ROLE,
        UPDATED_USER_DATA_ROLE,
    )
...
def test_user_route_validation_error(client, mock_db, auth_header):
    request_invalid_resource_validation_error(
        client, mock_db, auth_header, "post", URL_USER, INVALID_USER_DATA
    )
    request_invalid_resource_validation_error(
        client, mock_db, auth_header, "put", URL_USER_ID, INVALID_USER_DATA
    )
...
def test_user_route_unexpected_error(client, mock_db, auth_header):
    ...
    request_unexpected_error(
        client, mock_db, auth_header, "find_one_and_update", URL_USER_ID
    )
...
def test_user_route_resource_not_found_error(client, mock_db, auth_header):
    request_resource_not_found_error(
        client, mock_db, auth_header, "get", URL_USER_ID, VALID_USER_DATA, RESOURCE
    )
    request_resource_not_found_error(
        client, mock_db, auth_header, "put", URL_USER_ID, VALID_USER_DATA, RESOURCE
    )
    request_resource_not_found_error(
        client, mock_db, auth_header, "delete", URL_USER_ID, VALID_USER_DATA, RESOURCE
    )
