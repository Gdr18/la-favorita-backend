...
def request_unauthorized_set(
    ...
    updated_resource_data: dict = None,
):
    response = None
    if request == "post":
        response = client.post(url_resource, json=valid_resource_data, headers=header)
...
def request_invalid_resource_duplicate_key_error(
    client,
    mock_db,
    header,
    mocker,
...
def request_invalid_resource_validation_error(
    client, mock_db, header, method: str, url_resource: str, invalid_resource_data: dict
):
    response = None
    if method == "post":
